...
const Gallery = ({ initialData }) => {
    ...
    const projects = initialData ? initialData.flatMap(cat =>
        (cat.projects || []).map(p => ({
            id: p.id,
            title: p.title,
            category: cat.slug,
        ...
        // Fallback data (truncated for brevity, using existing structure)
        {
            id: 1,
        ...
        // ... (Simplified fallback for development if DB is empty but we want to see layout)
    ];
