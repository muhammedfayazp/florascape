...
@section('content')
    ...
    @php
        ...
        // Determine if the calculator section should be shown.
        // Always visible to logged-in admins; controlled by site setting for guests.
        $showCalculator = auth()->check()
            ? true
            : (bool) optional($siteSettings)->show_calculator;
    @endphp
    ...
    {{--
        Visibility logic:
        - If show_calculator = true  → visible to everyone
        - If show_calculator = false → hidden for guests, but admins see an "admin-only" notice + the widget
    --}}
        ...
        @elseif(auth()->check())
            ...
            <section class="calculator-section" style="position: relative;">
                <div style="position: absolute; top: 1rem; left: 50%; transform: translateX(-50%); z-index: 100;
                            background: #f59e0b; color: #1c1917; padding: 0.4rem 1.2rem; border-radius: 9999px;
                            font-size: 0.8rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase;
                            box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                    👁 Admin Preview – Hidden from Visitors
                </div>
                <div class="container" style="opacity: 0.65; filter: blur(1px); pointer-events: none;">
                    <div id="cost-calculator-root"
                         data-title="{{ $sections['cost_calculator']->title }}"
                         data-subtitle="{{ $sections['cost_calculator']->subtitle }}"
                         data-content='@json($sections['cost_calculator']->content)'></div>
        ...
        <section class="section">
            <div class="container grid md:grid-cols-2 gap-8 items-center">
                ...
                <div>
                    ...
                    <p style="color: #666; margin-bottom: 2rem; font-size: 1.1rem;">{{ $sections['why_choose_us']->subtitle }}</p>
